import {
  ...
  InternalServerError,
  UnauthorizedError,
} from "@/v0/errors/HttpError";
import { supabase } from "@/v0/lib/supabase";
...
export class AuthMiddleware {
      ...
      try {
        ...
        if (!hasAllowedRole) {
          throw new ForbiddenError(
            `Access denied. User '${userId}' has roles [${roleNames.join(", ") || "none"
  ...
  requirePermissions =
    (resourceName: string, requiredActions: string | string[]): RequestHandler =>
    async (req, res, next) => {
      try {
        const userId = this.getAuthenticatedUserId(req);
        const { roleNames, permissions } = await this.loadUserAccessProfile(userId);
        ...
        const missingActions = actions.filter(
          (actionName) => !this.hasPermission(permissions, resourceName, actionName),
        );
