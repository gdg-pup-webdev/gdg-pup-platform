...
export class DocsRouter {
  ...
  constructor() {
     ...
     */
    this.router.use("/openapispec.json", (req, res) => {
      res.setHeader("Content-Type", "application/json");
      res.send(swaggerSpec);
    });

    this.router.use("/postman-import.json", (req, res) => {
      res.setHeader(
        "Content-Disposition",
            ...
            items.forEach((item) => {
              ...
              const clearPlaceholders = (params: Array<any>) => {
                params?.forEach((p) => {
                  ...
                  const isPlaceholder =
                    /^(string|number|integer|boolean|<.*>)$/i.test(p.value);
                  if (isPlaceholder) {
                    p.value = "";
                  }
                });
              ...
              if (item.request?.url?.query)
                clearPlaceholders(item.request.url.query);
     ...
     */
    this.router.get("/stoplight", (req, res) => {
      res.send(`
    <!doctype html>
    <html lang="en">
      <head>
      ...
      <body style="height: 100vh; overflow-y: hidden;">
        <elements-api 
          apiDescriptionUrl="http://localhost:8000/docs/openapispec.json" 
          router="hash" 
          layout="sidebar"
        />
      </body>
  ...
  `);
     ...
     */
    this.router.get("/rapidoc", (req, res) => {
      res.send(`
...
<html>
  ...
  <body>
    <rapi-doc
      spec-url = "http://localhost:8000/docs/openapispec.json"
    > </rapi-doc>
  </body>
</html>
  `);
