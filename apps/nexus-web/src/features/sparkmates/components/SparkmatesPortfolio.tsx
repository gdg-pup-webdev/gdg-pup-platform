...
const FIGMA_ASSETS = {
  profileCard: "https://www.figma.com/api/mcp/asset/3b8f15f0-f794-48d7-90f6-7463afc9d894",
  profileRing: "https://www.figma.com/api/mcp/asset/323fd0a6-f443-4caf-abc2-779db7ae33f3",
  suggestedRing: "https://www.figma.com/api/mcp/asset/a40a5418-1468-4827-bd4c-4527aa87f5ea",
  customLinkGradient: "https://www.figma.com/api/mcp/asset/487a6240-2dc1-485b-b27c-b192f5ff9947",
  projectOne: "https://www.figma.com/api/mcp/asset/09b49507-b6fe-4d0f-9332-6444a16245d7",
  projectTwo: "https://www.figma.com/api/mcp/asset/902a5aa1-eaa6-4a5e-8b70-7dd296a0102f",
  projectThree: "https://www.figma.com/api/mcp/asset/c806405e-7622-4201-a669-939b69396d7f",
  badge: "https://www.figma.com/api/mcp/asset/298196a1-6987-40f4-b7ee-621502d4cac0",
};
...
function SocialGlyph({ type }: { type: "github" | "linkedin" | "website" }) {
  ...
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
      ...
      <path d="M3 12h18M12 3c2.8 2.5 2.8 15.5 0 18M12 3c-2.8 2.5-2.8 15.5 0 18" strokeLinecap="round" />
    </svg>
  );
}
...
function GradientProfilePicture({
  ...
  return (
    <div className="relative h-43 w-43 shrink-0">
      <img src={FIGMA_ASSETS.profileRing} alt="" aria-hidden className="absolute inset-0 h-full w-full object-contain" />
      <div className="absolute left-1/2 top-1/2 h-41 w-41 -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-full">
        <Avatar src={src} alt={alt} fallback={fallback} className="h-full w-full rounded-full" />
      </div>
    </div>
  );
}
  ...
  return (
    <article className="relative flex items-center pl-11.5">
      ...
      <div className="absolute left-0 top-1/2 h-23.5 w-23.5 -translate-y-1/2">
        <img src={FIGMA_ASSETS.suggestedRing} alt="" aria-hidden className="absolute inset-0 h-full w-full object-contain" />
        <div className="absolute left-1/2 top-1/2 h-21.5 w-21.5 -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-full">
          <Avatar src={avatarUrl} alt={name} fallback={name.charAt(0)} className="h-full w-full rounded-full" />
        </div>
      </div>
    </article>
  );
...
function Divider() {
  return <div className="h-px w-full bg-[linear-gradient(90deg,#0F2449_0%,#2A4F91_50%,#0F2449_100%)]" />;
}
  ...
  return (
    <article className="rounded-2xl border border-white/15 bg-[rgba(255,255,255,0.04)] p-5 shadow-[inset_0px_4px_16px_rgba(255,255,255,0.25)]">
      ...
      <Text variant="body-sm" className="mt-1 text-[#E5E5E5]">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean ullamcorper sed eros, non sollicitudin.
      </Text>
      <img src={image} alt="Project preview" className="mt-2 h-20 w-full rounded-md object-cover" />
    </article>
  );
}
...
export function SparkmatesPortfolio({
  ...
  if (isError || !profile) {
    const message = error instanceof Error ? error.message : "Unable to load sparkmates profile.";
  ...
  const showActivationGate = requiresActivation && profile.status !== "activated";
      ...
      <div className="min-h-screen bg-[#010B1D] px-6 pb-24 pt-40 text-white">
        <div className="mx-auto max-w-2xl rounded-3xl border border-white/15 bg-white/5 p-8 text-center">
          ...
          {isOwner ? (
            ...
            >
              {activateMutation.isPending ? "Activating..." : "Activate Sparkmates Profile"}
            </Button>
          ) : null}
        </div>
      </div>
  ...
  const avatarUrl = resolveAuthenticatedAvatar(user);
  ...
  const projectImages = [FIGMA_ASSETS.projectOne, FIGMA_ASSETS.projectTwo, FIGMA_ASSETS.projectThree];

  const socialLinks = [
    { key: "github", url: profile.portfolio?.github_url, label: "GitHub" },
    { key: "linkedin", url: profile.portfolio?.linkedin_url, label: "LinkedIn" },
    { key: "website", url: profile.portfolio?.portfolio_website_url, label: "Website" },
  ] as const;
      ...
      <div className="relative min-h-screen w-full overflow-hidden">
        ...
        <FadeInSection className="p-0" delay={0.02}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Button variant="default" size="sm" iconRight={viewIcon} className="px-3 py-1 text-white">
              Preview
            </Button>
          </div>
          ...
          <div className="mt-6 p-0">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex min-w-0 items-start gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <Text variant="body-sm" className="text-[#C1C7CD]">
                    {profile.portfolio?.year_and_program || "Program and Year not set"}
                  </Text>
                  <div className="mt-2 flex flex-wrap gap-2">
                    ...
                    <Badge variant={SPARK_BADGE.variantId as never}>{profile.gdg_id}</Badge>
                  </div>
                  <Text variant="body-sm" className="mt-2 max-w-130 text-[#E5E5E5]">
                    {profile.portfolio?.bio || "Share your story to let sparkmates know what you are building."}
                  </Text>
              ...
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-white" title="Menu" aria-label="Menu">
                  {burgerIcon}
                </Button>
                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-white" title="Edit" aria-label="Edit">
                  {editIcon}
                </Button>
              </div>
            </div>
          ...
          <div className="mt-8 space-y-8">
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <Text variant="heading-6" gradient="white-blue" weight="bold">Custom Button</Text>
                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-white" title="Edit Custom Button" aria-label="Edit Custom Button">
                  {editIcon}
                </Button>
              </div>
              <Text variant="body-sm" className="text-[#C1C7CD]">Add a custom button that appears on your profile.</Text>
                ...
                {customLinks.map((item, index) => (
                  ...
                  >
                    <ShineBorder
                      ...
                      className={starredCustomButtons.has(index) ? "opacity-100 transition-opacity duration-300" : "opacity-0 transition-opacity duration-300"}
                    />
                    <div className="flex items-start justify-between">
                      <div>
                        <Text variant="body-lg" className="text-white" weight="medium">{item.title}</Text>
                        <Text variant="body" className="text-[#E5E5E5]">{item.url}</Text>
                      </div>
                      <Button
                        variant="ghost"
                        size="sm"
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <Text variant="heading-6" gradient="white-blue" weight="bold">Projects</Text>
                <Button variant="default" size="sm" className="text-white" iconRight={viewIcon}>View All</Button>
              </div>
              <Text variant="body-sm" className="text-[#C1C7CD]">Feature your best works to highlight your skills.</Text>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <Text variant="heading-6" gradient="white-blue" weight="bold">GDG Impact</Text>
                <Button variant="default" size="sm" className="text-white" iconRight={viewIcon}>View</Button>
              </div>
              <Text variant="body-sm" className="text-[#C1C7CD]">Track your milestones and growth within GDG.</Text>
              <div className="grid grid-cols-3 gap-4">
                {["Study Jam", "Workshop", "Hackathon"].map((label) => (
                  <div key={label} className="rounded-2xl border border-white/20 bg-[rgba(255,255,255,0.04)] px-5 py-6 shadow-[inset_0px_4px_16px_rgba(255,255,255,0.25)]">
                    <Text variant="heading-5" align="center" gradient="white-yellow" weight="bold">00</Text>
                    <Text variant="body-sm"  align="center" className="text-white">{label}</Text>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <Text variant="heading-6" gradient="white-blue" weight="bold">Badges</Text>
                <Button variant="default" size="sm" className="text-white" iconRight={viewIcon}>View All</Button>
              </div>
              <Text variant="body-sm" className="text-[#C1C7CD]">Unlock exclusive collectibles by attending events.</Text>
              <div className="grid grid-cols-3 gap-4">
                {badgeCards.map((badge) => (
                  <div key={badge} className="rounded-2xl border border-white/20 bg-[rgba(255,255,255,0.04)] p-4 text-center shadow-[inset_0px_4px_16px_rgba(255,255,255,0.25)]">
                    <img src={FIGMA_ASSETS.badge} alt="Badge" className="mx-auto h-20 w-20 object-cover" />
                    <Text variant="body-sm" className="mt-2 text-white"  align="center">Badge Name</Text>
                  </div>
                ))}
              </div>
            </section>
        ...
        <FadeInSection  delay={0.1}>
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            ...
            <Button variant="default" size="sm" iconRight={viewIcon} className="text-white">
              View All
            </Button>
          </div>
