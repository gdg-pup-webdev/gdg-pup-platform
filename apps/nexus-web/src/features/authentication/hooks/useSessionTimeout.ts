 ...
 * Silently refreshes token if user was active within the last 30 minutes
 * Shows warning if user has been completely idle for more than 30 minutes
...
export const useSessionTimeout = (config: SessionTimeoutConfig = {}) => {
  ...
  const checkSessionTimeout = useCallback(async () => {
    ...
    if (timeUntilTokenExpires <= 0) {
      console.log("Token expired: logging out user", timeUntilTokenExpires, timeSinceLastActivity, decodedToken);
      if (config.onTimeout) {
        config.onTimeout();
      }
      logout();
    ...
    if (timeUntilTokenExpires <= WARNING_THRESHOLD_MS) {
      // 2a. If there was activity in the last 30 minutes, silently refresh
      if (timeSinceLastActivity < INACTIVITY_THRESHOLD_MS) {
        console.log("Approaching expiry but user has been active. Silently refreshing...", timeUntilTokenExpires, timeSinceLastActivity, decodedToken);
        try {
          await refreshToken();
        } catch (error) {
      ...
      // 2b. If totally idle for > 30 minutes, show warning modal
      if (!warningShownRef.current && isAuthenticatedRef.current) {
        warningShownRef.current = true;
        console.log(
          `Session warning: ${Math.round(timeUntilTokenExpires / 1000 / 60)} minutes remaining`, timeUntilTokenExpires, timeSinceLastActivity, decodedToken
        );
        if (config.onWarning) {
