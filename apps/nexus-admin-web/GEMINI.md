...
Welcome to the `nexus-admin-web` codebase. This is a Next.js 16 (App Router) application that serves as the primary frontend for the GDG PUP Nexus platform.
...
Ensure you have `pnpm` installed and are in the `apps/nexus-admin-web` directory.

### Development
```bash
pnpm dev
```
Starts the development server on [http://localhost:4000](http://localhost:4000).
