...
export const SignupFlow = () => {
  ...
  if (step === 1) {
    return (
      <form className="space-y-6" onSubmit={handleInitiate}>
        {initError && (
          <div className="rounded-md bg-red-50 p-4 text-sm text-red-700">{initError.message}</div>
        )}
        <div>
        ...
        <button
          ...
          className="flex w-full justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-indigo-400"
        >
  ...
  return (
    <form className="space-y-6" onSubmit={handleFinalize}>
      ...
      {finalError && (
        <div className="rounded-md bg-red-50 p-4 text-sm text-red-700">{finalError.message}</div>
      )}
      <div>
