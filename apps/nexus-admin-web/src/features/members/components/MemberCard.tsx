    ...
    >
      ...
      <div className="flex h-32 items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100/50">
        <div className="relative flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-white shadow-sm border border-white">
          {member.avatarUrl ? (
            <Image src={member.avatarUrl} alt={fullName} fill className="object-cover" />
          ) : (
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-auto space-y-2.5">
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span className="line-clamp-1">{member.department || "General"}</span>
          </div>
          
          <div className="flex items-center gap-2 text-xs text-gray-500">
            <GraduationCap size={14} className="text-gray-400" />
            <span className="line-clamp-1">{member.yearLevel ? `${member.yearLevel} Year` : ""} {member.program || ""}</span>
          </div>
