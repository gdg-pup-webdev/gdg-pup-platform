...
3.  **The "Tap Assembly Line":**
    ...
    *   Person B: Taps card on reader -> *Beep! (Written)*.
...
This script generates the IDs in code (Client-Side) and pushes them to *both* the CSV file and the Database. Not relying on DB auto-generation ensures perfect sync.
